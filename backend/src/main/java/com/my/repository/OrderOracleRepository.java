...
public class OrderOracleRepository implements OrderRepository {
	...
	private void insertLines(Connection con, List<OrderLine> lines) throws SQLException{
		...
		for(OrderLine line: lines) {
			String prodNo = line.getOrderP().getProductNo();
	...
	public List<OrderInfo> selectById(String orderId) throws FindException {
		...
		try {
            ...
            while (rs.next()) {
            	...
            	Date orderDate = null;
            	try {
					orderDate = new SimpleDateFormat("yyyy-mm-dd").parse(listForAllElementsOfOrderInfo.get(2));
    			...
    			String selectLineSQL = 
    					"SELECT * FROM order_line WHERE order_no= ?";
